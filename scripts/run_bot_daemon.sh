#!/bin/bash

# Скрипт для запуска бота как демона

BOT_PID_FILE="bot.pid"
BOT_LOG_FILE="bot.log"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "🤖 Запуск бота Pachka как демона..."

# Переходим в директорию скрипта
cd "$SCRIPT_DIR"

# Проверяем, не запущен ли уже бот
if [ -f "$BOT_PID_FILE" ]; then
    PID=$(cat "$BOT_PID_FILE")
    if ps -p $PID > /dev/null 2>&1; then
        echo "⚠️ Бот уже запущен с PID: $PID"
        echo "🛑 Для остановки: kill $PID"
        exit 1
    else
        echo "🧹 Удаляем старый PID файл"
        rm -f "$BOT_PID_FILE"
    fi
fi

# Проверяем наличие переменной окружения
if [ ! -f ".env" ]; then
    echo "❌ Файл .env не найден!"
    echo "📝 Создайте файл .env с содержимым:"
    echo "   PACHKA_TOKEN=ваш_токен_здесь"
    exit 1
fi

# Активируем виртуальное окружение
if [ -d "find_sims_env" ]; then
    echo "🔧 Активируем виртуальное окружение..."
    source find_sims_env/bin/activate
fi

# Устанавливаем waitress если не установлен
echo "📦 Проверяем зависимости..."
pip install waitress

# Запускаем бота в фоновом режиме с полным отключением от терминала
echo "🚀 Запускаем бота..."
nohup python3 start_bot.py > "$BOT_LOG_FILE" 2>&1 &
BOT_PID=$!

# Сохраняем PID
echo $BOT_PID > "$BOT_PID_FILE"

echo "✅ Бот запущен с PID: $BOT_PID"
echo "📊 Логи: tail -f $BOT_LOG_FILE"
echo "🛑 Остановка: kill $BOT_PID"
echo "🔍 Статус: ps aux | grep $BOT_PID"

# Ждем немного и проверяем, что бот запустился
sleep 5
if ps -p $BOT_PID > /dev/null 2>&1; then
    echo "✅ Бот успешно запущен и работает"
    echo "🌐 Проверьте доступность: curl http://91.217.77.71:5000/health"
else
    echo "❌ Ошибка запуска бота"
    echo "📋 Проверьте логи: cat $BOT_LOG_FILE"
    rm -f "$BOT_PID_FILE"
    exit 1
fi 